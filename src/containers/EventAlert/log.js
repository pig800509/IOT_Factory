      ...
      <Layout className="layout" style={{ height: "100vh",overflowY:"scroll"}}>
        ...
        <Content style={{
          ...
          {logTabs.map((value,index)=>{
            return <TabPane tab={value.title} key={index}><LogTable sourceApi={`http://apilog.iotcomm.net/api/logData?category=${value.type}&limit=10`} columns={logColumns}/></TabPane>
          })}
          </Tabs >
        </Content>
        <Footer/>
